...
\subsection{Instalar o PyTorch (com ou sem GPU) e o pacote}\label{sec:instalar}
\begin{lstlisting}
python -m pip install --upgrade pip
# (a) COM GPU NVIDIA -- veja abaixo como escolher o indice:
...
\begin{alerta}[O \code{cuXXX} não é a versão do seu driver]
O número no \code{--index-url} é o \emph{build} do PyTorch, não o CUDA instalado na máquina. Ele precisa cobrir a \emph{arquitetura} da sua GPU: placas RTX~50xx são Blackwell (\code{sm\_120}) e exigem \code{cu128} ou superior --- um build \code{cu124} instala sem erro e depois falha em toda operação. RTX~40xx aceita \code{cu121+}; RTX~30xx, \code{cu118+}. Além disso, cada índice só publica \emph{wheels} para algumas versões de Python: com \textbf{Python 3.14} hoje só o \code{cu130} tem. Na dúvida, abra \code{https://download.pytorch.org/whl/cu130/torch/} e procure a linha com o seu \code{cp3XX} e sistema operacional.
\end{alerta}

Confirme --- e note que \code{cuda.is\_available()} pode dizer \code{True} com um build incompatível, então o teste tem que \emph{calcular} alguma coisa:
\begin{lstlisting}
...
\end{lstlisting}
Se aparecer \code{no kernel image is available for execution on the device}, o build não cobre a sua GPU: volte ao alerta acima e instale de um índice mais novo.
...
\code{torch.cuda.is\_available()} $=$ \code{False} & Instalou o torch de CPU (confirme: \code{torch.version.cuda} vem \code{None}). Desinstale (\code{pip uninstall -y torch torchvision}) e reinstale pelo índice certo da Seção~\ref{sec:instalar}. Confira o driver com \code{nvidia-smi}. \\
\code{no kernel image is available} & O build do torch não cobre a arquitetura da sua GPU (típico de RTX~50xx com \code{cu124}). Reinstale de um índice \code{cu128} ou superior. \\
